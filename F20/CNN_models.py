...
def twoLayerCNN(input_size, filters1=32, filters2=64, dropout=True, maxpooling=True):
    ...
    '''
        Implement a two layer convolutional network: input-->conv-->(pooling-->dropout)-->
        conv-->(pooling-->dropout)-->flatten-->dense-->output

        Argument:
            input_size: input shape
            filters1: the number of filter for the first convolution layer
            filters2: the number of filter for the second convolution layer
            dropout: boolean which indicates whether to add dropout layer
            maxpooling: boolean which indicates whether to add pooling layer
