...
from scipy.signal import butter,lfilter,filtfilt,find_peaks,resample
import pywt
...
# def R_peak_detector_WT(original_signal,fs=240):


#     # calculate the wavelet transform coefficients based on a 4-level undecimated DWT using sym4 
#     wtc =  pywt.wavedec(original_signal,'sym4',mode='ze', level=4)

#     d4 = wtc[1] # 4-level detail coefficient
#     d3 = wtc[2] # 3-level detail coefficient

#     y = pywt.idwt(None, d4,'sym4')
#     y += pywt.idwt(None,d3, 'sym4')
...
#     # calculate the indices of R peaks that satisfy all given conditions.
#     r_peak = find_peaks(y,height = 8*avg_y ,distance=50) 
...
def dynamic_window_segmentation(r_peaks, finalidx, before = 0.5, after = 0.5):
    ...
    ##Handle first r_peak
    segments.append((int(max(r_peaks[0]-rr_lengths[0]*before, 0)),int(r_peaks[0]+rr_lengths[0]*after))) #ensure we stay in window
    
    ##Segment the middle peaks
    for peak_idx in range(1, len(r_peaks)-1):
        segments.append((int(r_peaks[peak_idx] - rr_lengths[peak_idx-1]*before), int(r_peaks[peak_idx]+rr_lengths[peak_idx]*after)))

    ##Handle Last segment
    segments.append((int(r_peaks[-1] - rr_lengths[-1]*before), int(min(r_peaks[-1] + rr_lengths[-1]*after,finalidx)))) #ensure we stay in window
...
def resample_segments(segment_idxs, time, signals, num_samples=None):
    ...
    for start, end in segment_idxs:
        ...
        for label, signal in signals.items():
            f = interp1d(time[start:end],signal[start:end],fill_value='extrapolate')
            signew = f(tnew)
...
# ecg1 = P1_B4['GE_WAVE_ECG_1_ID'][:2400]

# ecg1_rpeak = R_peak_detector_WT(ecg1)

# start = 4800
# window = 2400
# end = start+window

# ecg1 = schmidt_spike_removal(P1_B4['GE_WAVE_ECG_1_ID'][start:end])
# time = P1_B4['time'][start:end]

# rpeak_idxs = pan_tompkins_detector(ecg1)
# rpeak_times = time[rpeak_idxs]
# #ecg1_rpeak_adjusted = [start + i for i in ecg1_rpeak]
# #print("Peaks: ", rpeak_times)

# #Segmentation
# segment_idxs = dynamic_window_segmentation(rpeak_idxs, window)

# #Resampling
# #tsamp,dic = resample_segments(segment_idxs, time, {'ECG1': ecg1})
# #ssamp = dic['ECG1']

# plt.plot(time, ecg1)
# plt.plot(rpeak_times ,0.3+np.zeros(len(rpeak_times)),'o', color='red')
# plt.vlines(segments, min(ecg1), max(ecg1),linestyles='dotted')
# plt.show()

# #Resampling plots
# fig, axs = plt.subplots(len(t),figsize=(4,20))
# for i in range(len(tsamp)):
#     axs[i].plot(tsamp[i], ssamp[i])
