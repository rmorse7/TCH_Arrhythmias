...
# """
    ...
    for l in label:
        current_file_paths = glob(os.path.join(data_dir, '*_'+l+'.npy'))
        current_train_paths,current_val_paths = train_test_split(current_file_paths, test_size=val_size)
...
def data_batch_generator(img_paths, batch_size, label_dict = {'PP':0,'PS':0,'PJ':1,'PO':1,'PJRP':1,},res=True):
# '''
...
#     res: boolean. if resize is true then resize the original images
...
# '''
    ...
    while True:
        ...
        yield img_batch, label_batch # x_train, y_train

def get_train_valid_generator(data_dir, batch_size, val_size = 0.15):

    '''
    Return data generator for training and validation
    Arguments:
        data_dir: the path of folder containing cwt features
        batch_size: batch size
    ...
    train_img_paths, val_img_paths = split_train_val(data_dir,val_size=val_size)
...
##########test\
# data_dir = 'I:/COMP549/cwt_features_images'
# get_train_valid_generator(data_dir,batch_size=4)